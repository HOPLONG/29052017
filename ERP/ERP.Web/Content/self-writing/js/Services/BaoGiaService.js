app.service('baogiaService', function ($http) {
    this.load_dondukien = function (madk) {
        return $http.get('http://27.72.144.148:8003/api/Api_BaoGia_DonHangDuKien/BH_DON_HANG_DU_KIEN/' + madk).then(function (response) {
            return response.data;
        });
    };
    this.load_baogiatheodukien = function (madk) {
        return $http.get('http://27.72.144.148:8003/api/Api_BaoGia/BaoGiaTheoDuKien/' + madk).then(function (response) {
            return response.data;
        });
    };
    ...
    this.copynewbg = function (thongtinchung) {
        return $http.post('http://27.72.144.148:8003/api/Api_ChiTietBaoGia/CopyNewBaoGia', thongtinchung);
    };
});